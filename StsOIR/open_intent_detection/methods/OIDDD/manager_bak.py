...
class ADBManager:
    ...
    def __init__(self, args, data, model, logger_name = 'Detection'):
        ...
        if args.train:
            ...
            self.param_ab = np.load(os.path.join(args.method_output_dir, 'param_ab.npy'))
            self.param_ab = torch.from_numpy(self.param_ab).to(self.device)
    ...
    def train(self, args, data):  
        ...
        self.param_ab = F.softplus(criterion_boundary.param_ab)
        optimizer = torch.optim.Adam(criterion_boundary.parameters(), lr = args.lr_boundary)
        # self.centroids = self.centroids_cal(args, data)

        best_eval_score, best_delta, best_centroids , best_param_ab = 0, None, None, None
        wait = 0
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            if eval_score > best_eval_score:
                ...
                best_param_ab = criterion_boundary.param_ab
                best_eval_score = eval_score
        ...
        if args.save_model:
            ...
            np.save(os.path.join(args.method_output_dir, 'param_ab.npy'), self.param_ab.detach().cpu().numpy())
    ...
    def open_classify(self, data, features):
        ...
        k = self.param_ab[preds]
        z = (features-self.centroids[preds]).view(features.shape[0], 2, features.shape[1]//2)
        z = torch.norm(z, 2, 2)
