...
class DeepTestdManager:
    ...
    def __init__(self, args, data, model, logger_name = 'Discovery'):
        ...
        pretrain_manager = PretrainDeepTestManager(args, data, model)  
    ...
    def train(self, args, data): 
        ...
        # for epoch in trange(int(10), desc="Epoch"):  
            ...
            # gm = GaussianMixture(n_components=self.num_labels).fit(feats)
            # eval_score = silhouette_score(feats, gm.means_)
            

            tmp_k = 0.01

            penalty = balance_penalty(km.labels_, self.num_labels, tmp_k)
            balance_score = eval_score - penalty

            
            print('Silhouette: {:.5f}, Balance: {:.5f}, Penalty: {:.5f}'.format(
                    eval_score, balance_score, penalty))

            if epoch > 0:
                
                eval_results = {
                    'train_loss': tr_loss,
                    'cluster_silhouette_score': balance_score,
            ...
            '''
            TODO  使用balance_penalty替换原 eval_score 选择最优模型
            if balance_score > best_eval_score:
                best_model = copy.deepcopy(self.model)
                wait = 0
                best_eval_score = balance_score
